...
Для WB Stream панель использует device-assisted creator: **Провайдеры →
Подключить / сменить WB → Создать QR для телефона**. QR создаёт постоянную
привязку Android к Manager. Android входит в WB локально, создаёт звонки обычным
интерфейсом и отправляет Manager только готовую ссылку приглашения. Cookies и
аккаунтные WB-токены остаются внутри Android WebView. Manager проверяет ссылку,
запускает relay как участника готовой комнаты и при завершении звонка запрашивает
у Android новое приглашение. Для Telemost и Dion пока нужен соответствующий файл
из таблицы; автоматический вход через панель для них ещё не реализован.
