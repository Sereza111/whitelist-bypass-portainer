...
| Переменная | Значение |
|---|---|
| `WLB_IMAGE` | `ghcr.io/sereza111/whitelist-bypass-portainer:v0.5.0-alpha.21` |
...
Для WB Stream панель имеет отдельный device-assisted вход: **Провайдеры →
Подключить / сменить WB → Создать QR для телефона**. Актуальный Android-клиент
открывает WB на пользовательской сети, получает allowlisted сессию и отправляет
её по HTTPS с одноразовым bearer token. Сервер проверяет её через WB `slide-v3`
и сохраняет `/data/managed-secrets/cookies-wbstream.json` с правами `0600`.
Серверный Chromium не используется: WB отвечает VPS кодом `HTTP 498`. Номер,
код, pairing token и cookies не возвращаются status API и не попадают в логи. Для
