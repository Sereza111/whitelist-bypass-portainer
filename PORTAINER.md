...
В разделе **Клиенты** администратор создаёт технические и собственные профили.
`Лимит сессий` ограничивает только этот профиль, а `MAX_SESSIONS` — весь
сервер. Профиль с включёнными `Enabled` и **Всегда запущен** автоматически
создаёт независимую сессию после перезапуска контейнера/VPS и проверяется
повторно каждые 30 секунд. Для постоянной остановки отключите профиль или
снимите **Всегда запущен**. Ссылка, журнал и METRICS доступны в разделе
**Сессии**.

Для самостоятельного доступа откройте **Настройки → Приглашение в кабинет** и
передайте пользователю одноразовую HTTPS-ссылку. После регистрации он входит на
`/portal`, создаёт и запускает только собственные профили и получает свою ссылку
для Android. Чужие профили и сессии, Providers, cookies, события и глобальные
настройки пользовательскому кабинету недоступны. Регистрация без приглашения
отключена.
...
Если QR неудобно передавать, в этом же окне скопируйте HTTPS-адрес Manager и
одноразовый код и введите их в Android вручную.
