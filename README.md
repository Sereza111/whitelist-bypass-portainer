...
4. Задайте `WLB_IMAGE`, `PANEL_USERNAME`, новый `PANEL_PASSWORD`,
   `WLB_SECRETS_DIR`, `MAX_SESSIONS=4`, `VIDEO_RELIABILITY=auto` и
   `KCP_PROFILE=balanced`. Для автоматической доставки новых VK-ссылок также
   задайте `VK_PEER_ID` — цифровой ID вашего личного VK-аккаунта.
...
- [Полный отчёт и контекст передачи проекта](docs/PROJECT_REPORT_2026-07-21.md)
- [PDF-версия отчёта](output/pdf/whitelist-bypass-project-report-2026-07-21.pdf)
...
Панель хранит клиентские профили в `/data/control-plane.json`. Для каждого
профиля можно выбрать провайдера, KCP-профиль, лимит одновременных сессий,
срок действия и состояние доступа. Каждый запуск получает отдельный Creator,
каталог, ссылку, диагностику и журнал; общий предел задаёт `MAX_SESSIONS`.

### Автовосстановление VK-звонка

Начиная с `v0.5.0-alpha.5`, Manager перезапускает оборвавшийся VK Creator с
ограниченным backoff, создаёт новый звонок и отправляет подписанную ссылку в
VK. Android проверяет HMAC-подпись, номер поколения и срок сообщения, затем
обновляет профиль и переподключается.

Для надёжных уведомлений используйте отдельный VK-аккаунт в server cookies, а
в `VK_PEER_ID` укажите ID аккаунта на телефоне. В панели запустите профиль,
нажмите **В телефон**, вставьте блок в Android через **Add connection**, затем
разрешите **Settings → Automatic recovery** читать уведомления. Recovery key
считайте паролем: не публикуйте скопированный блок и не прикладывайте его к
логам. Подробная схема и ограничения описаны в [документации панели](docs/PANEL.md#автоматическое-восстановление-vk).

### Вход серверного VK по QR

В `v0.5.0-alpha.6` ручной экспорт cookies для VK больше не обязателен. Нажмите
**Войти через QR** в блоке `VK credentials`, отсканируйте код телефоном под
отдельным серверным аккаунтом и подтвердите вход. Manager запускает
изолированный Chromium только на время авторизации, проверяет сессию, атомарно
сохраняет cookies в `/data/managed-secrets/cookies-vk.json` с правами `0600` и
закрывает браузер. Логин и пароль через панель не передаются.

Cookies, полученные через QR, имеют приоритет над старым read-only файлом из
`WLB_SECRETS_DIR`; он остаётся резервным. После смены аккаунта остановите
текущую Creator-сессию и запустите профиль снова. QR-браузер включён в образы
`amd64` и `arm64`; legacy-образ `386` продолжает поддерживать импорт файла.
