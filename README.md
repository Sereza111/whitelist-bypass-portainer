...
Текущий кандидат: **v0.5.0-alpha.39**. До публикации immutable tag используйте branch artifacts только для проверки matching Android + server.

Docker image:

```text
ghcr.io/sereza111/whitelist-bypass-portainer:v0.5.0-alpha.39
```
...
|---|---|
| `WLB_IMAGE` | `ghcr.io/sereza111/whitelist-bypass-portainer:v0.5.0-alpha.39` |
...
При активных белых списках публичный адрес Manager может быть недоступен до
подъёма туннеля. Поэтому Manager автоматически входит в последнюю ранее
проверенную WB-комнату, а Android при недоступном control plane сначала запускает
ту же сохранённую комнату. После подключения creator polling и profile-sync идут
к Manager через локальный SOCKS работающего туннеля. Не создавайте вручную новую
одиночную комнату вместо bootstrap: её идентификатор ещё неизвестен серверу.
Самая первая привязка, для которой общей проверенной комнаты пока нет, требует
однократного доступного канала до Manager (например, обычного Wi-Fi).
...
```text
[build] version=0.5.0-alpha.39 commit=... built=...
```
