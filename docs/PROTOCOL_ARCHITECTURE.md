...
Legacy WB KCP сохраняет прежний wire path, но по умолчанию использует
ограниченный Auto вместо Fast: стартовое send window 256, диапазон 256–512 и
корректировка раз в две секунды по подтверждённым ACK-сегментам и реальному
заполнению. Negotiated VK KCP также по умолчанию использует Auto. Balanced
остаётся ручным окном `512/512`, Stable использует `256/256`, fast —
`2048/2048`. Auto рекламируется отдельной capability для negotiated path: peer
