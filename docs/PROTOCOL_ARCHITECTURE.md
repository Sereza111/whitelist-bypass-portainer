...
Legacy WB KCP сохраняет прежний wire path и использует Balanced `512/512`.
Полевой alpha.24 показал, что общий Auto на WB застревает на стартовом окне 256
и ограничивает high-RTT carrier примерно до 0.5 Mbps. Negotiated VK KCP по
умолчанию продолжает использовать Auto с диапазоном 256–512. Stable использует
`256/256`, fast —
