...
**Контекстное меню / инлайн-действия — НЕ реализованы.** Идея на будущее:
правый клик по карточке клиента/сессии → меню (Запустить / Стоп / В телефон /
Копировать ссылку / Изменить / Удалить) вместо ряда кнопок; это разгрузит
карточки и приблизит к 3X-UI. Точки входа: `renderProfiles()` и
`renderSessions()` в `headless/manager/web/app.js`, обработчики навешиваются в
`bindDynamicActions()`. Сохранять существующие классы кнопок
(`.start-client`, `.stop-session` и т.д.), на которые завязан код.
