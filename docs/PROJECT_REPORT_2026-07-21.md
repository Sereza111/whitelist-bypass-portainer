...
- **Последний полностью опубликованный релиз:** `v0.5.0-alpha.7`
- **Готовящийся matching-релиз:** `v0.5.0-alpha.9`
...
| `v0.5.0-alpha.9` | Кандидат: полный Gothic UX, постоянная APK-подпись, двусторонний KCP profile и reliable DNS lane |

## 5. Проверки и подтверждённые результаты

Для кандидата `v0.5.0-alpha.9` локально выполнены:
...
Перед публикацией ещё обязательны:

- установка Android signing secrets в GitHub Actions после подтверждения
  владельца репозитория;
- branch CI, затем tag CI для Android/Windows/Docker;
- проверка подписи APK по публичному SHA-256 fingerprint;
- проверка Release assets, SHA256 и GHCR `amd64`/`arm64`/`386` manifests;
...
Пока `v0.5.0-alpha.9` не опубликован и его CI/assets не проверены, актуальным
production-like образом остаётся `alpha.7`. После успешного alpha.9 релиза
сервер, Android и Windows необходимо обновить вместе, иначе новые negotiated
возможности останутся выключены.
